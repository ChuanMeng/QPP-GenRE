...
For the paper titled "[Query Performance Prediction using Relevance Judgments Generated by Large Language Models](https://arxiv.org/abs/2404.01012)", we propose a new query performance prediction (QPP) framework, `QPP-GenRE`, which first automatically generates relevance judgments for a ranked list for a given query, and then regard the generated relevance judgments as pseudo labels to compute different IR evaluation measures. `QPP-GenRE` can be integrated with various methods for judging relevance. 
...
For the paper titled "[Can We Use Large Language Models to Fill Relevance Judgment Holes?](https://arxiv.org/abs/2405.05600)", we fine-tune Llama 3 for generating relevance judgments in the context of conversational search.
